...
async function fundContract(contractAddress, links) {
    ...
    if ("abi" in contractABI) {
        ...
        return LinkTokenContract.transfer(contractAddress, (links * Math.pow(10, 18)).toString())
            .then(tx => {
...
function getEligiblePoolData() {
    ...
    const query = `{
        pools(where:{dataProvider: "0x8c244f0b2164e6a3bed74ab429b0ebd661bb14ca", expiryTime_lt: ${timestampNow}, expiryTime_gte: ${timeStampBefore7Days}, statusFinalReferenceValue: "Challenged", statusFinalReferenceValue: "Open"}) {
        id
        referenceAsset
        }
    }`

    return axios.post("https://api.thegraph.com/subgraphs/name/divaprotocol/diva-goerli-new", {
        'query': query
    })
        .then(response => {
...
async function sendRequest(dataset_code, selected_band, image_scale, start_date, end_date, geometry) {
    agg_x = "agg_mean"
